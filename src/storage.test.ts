...
test("loadConfig tolerates UTF-8 BOM in accounts.json", async () => {
  const originalHome = process.env.HOME
  const originalUserProfile = process.env.USERPROFILE
  const originalCwd = process.cwd()

  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "storage-config-test-"))
  const tempHome = path.join(tempRoot, "home")
  const tempProject = path.join(tempRoot, "project")
  fs.mkdirSync(tempHome, { recursive: true })
  fs.mkdirSync(tempProject, { recursive: true })
  ...
  try {
    ...
    fs.writeFileSync(configPath, withBom, "utf8")

    const modUrl = new URL(`./storage.ts?cachebust=${Date.now()}`, import.meta.url)
    const { loadConfig } = await import(modUrl.href)

    const config = loadConfig()
    assert.equal(config.schemaVersion, 2)
    assert.equal(Array.isArray(config.accounts), true)
  } finally {
    process.chdir(originalCwd)
    if (originalHome === undefined) delete process.env.HOME
    else process.env.HOME = originalHome
