...
test("auth sync append mode dedupes and keeps previous provider entries", async () => {
  ...
  try {
    ...
    const manager: {
      ...
      switchToAccount(accountId: string): { account: Account | null; reason?: string }
    ...
    const invalidAccountId = "manual-openai-invalid"
    const configWithInvalid = JSON.parse(fs.readFileSync(configPath, "utf8")) as {
    ...
    const existingAccounts = Array.isArray(configWithInvalid.accounts) ? configWithInvalid.accounts : []
    configWithInvalid.accounts = [
      ...existingAccounts,
      {
        id: invalidAccountId,
        name: "Manual Invalid Account",
        provider: "openai",
        priority: 999,
        credentials: {
          authType: "api_key",
    ...
    writeJson(configPath, configWithInvalid)

    manager.reload()

    const switchInvalid = manager.switchToAccount(invalidAccountId)
    assert.equal(switchInvalid.account, null)
    assert.equal(switchInvalid.reason, "invalid")

    const configAfterInvalid = JSON.parse(fs.readFileSync(configPath, "utf8")) as {
      accountMarkers?: Record<string, { status: string; rateLimitUntil?: string }>
    }
    assert.equal(configAfterInvalid.accountMarkers?.[invalidAccountId]?.status, "invalid")

    const summary = manager.getStatusSummary()
    assert.match(summary, /invalid credentials/i)
  } finally {
